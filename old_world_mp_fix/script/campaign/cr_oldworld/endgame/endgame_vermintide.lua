-- MP-FIX: Replaced pairs() with sorted iteration for multiplayer determinism (3 instances)
endgame_vermintide = {
	...
	skaven_factions = {
		wh2_main_skv_clan_mors      = "cr_oldworld_region_karag_lhune", -- Queek
		wh2_main_skv_clan_pestilens = "cr_oldworld_region_dragon_fang_mount", -- Skrolk
		wh2_dlc09_skv_clan_rictus   = "cr_oldworld_region_crookback_mountain", -- Tretch
		wh2_main_skv_clan_skryre   = "cr_oldworld_region_skavenblight", -- Ikit
		wh2_main_skv_clan_eshin    = "cr_oldworld_region_gnaw_pit", -- Snikch
		wh2_main_skv_clan_moulder  = "cr_oldworld_region_hell_pit", -- Throt
	},
...
function endgame_vermintide:trigger()
	-- MP-FIX #1: Sort keys to ensure deterministic iteration order
	local sorted_faction_keys = {}
	for faction_key in pairs(self.skaven_factions) do
		table.insert(sorted_faction_keys, faction_key)
	end
	table.sort(sorted_faction_keys)

	local potential_skaven = {}
	for i = 1, #sorted_faction_keys do
		local faction_key = sorted_faction_keys[i]
...
function endgame_vermintide:final_invasion()
	-- MP-FIX #2: Sort keys to ensure deterministic iteration order
	local sorted_faction_keys = {}
	for faction_key in pairs(self.skaven_factions) do
		table.insert(sorted_faction_keys, faction_key)
	end
...
function endgame_vermintide:expand_under_empire()
	-- MP-FIX #3: Sort keys to ensure deterministic iteration order
	local sorted_faction_keys = {}
	for faction_key in pairs(self.skaven_factions) do
		table.insert(sorted_faction_keys, faction_key)
	end
...
function endgame_vermintide:add_marker()
	local pos_x, pos_y = cm:find_valid_spawn_location_for_character_from_settlement("wh_main_emp_empire", "cr_oldworld_region_ubersreik", false, true, 9)
		...
		for i = 1, #human_factions do
			...
			if faction:subculture() == "wh_main_sc_emp_empire" or faction:subculture() == "wh_dlc05_sc_wef_wood_elves" or faction:subculture() == "wh_main_sc_dwf_dwarfs" then
				...
				cm:trigger_incident_with_targets(
					...
					0,
					0,
					cm:get_region("cr_oldworld_region_ubersreik"):cqi(),
					0
				)
			end
		end
...
function endgame_vermintide:generate_ubersreik_army(faction_key)
	...
	local pos_x, pos_y = cm:find_valid_spawn_location_for_character_from_settlement(faction_key, "cr_oldworld_region_ubersreik", false, true, 10)
	cm:disable_event_feed_events(true, "wh_event_category_character", "", "")
	...
	cm:create_force(
		...
		function(cqi)
			...
			for i = 1, #agents do
				local agent_x, agent_y = cm:find_valid_spawn_location_for_character_from_settlement(faction_key, "cr_oldworld_region_ubersreik", false, true, 10)
